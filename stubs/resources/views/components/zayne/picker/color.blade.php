...
<div class="zayne-picker-wrap {{ $compTheme }}" id="{{ $pid }}" style="{{ $style ?? '' }}">
  <style>
    ...
    #{{ $pid }} .zp-card { width: 280px; border-radius: 14px; padding: 14px; display: flex; flex-direction: column; gap: 12px; background: var(--zp-bg); border: 1px solid var(--zp-border); box-shadow: var(--zp-shadow); user-select: none; transition: width 0.3s ease; }
