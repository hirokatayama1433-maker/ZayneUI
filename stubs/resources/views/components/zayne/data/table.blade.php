            ...
            <tr class="bg-[var(--zayne-color-base-200)]">
                ...
                @if ($selectable)
                    <th class="w-10 px-4 py-3">
                        <x-zayne.form.checkbox name="select_all" size="sm" />
                    </th>
                @endif
            ...
            @if ($loading)
                ...
                @for ($i = 0; $i < 5; $i++)
                    <tr class="{{ $rowClasses }}">
                        @if ($selectable)
                            <td class="px-4 py-3">
                                <x-zayne.display.skeleton shape="box" width="16px" height="16px" />
                            </td>
                        @endif
                        @foreach ($columns as $col)
                            <td class="px-4 py-3">
                                <x-zayne.display.skeleton shape="line" width="{{ $loop->index === 0 ? '60%' : '80%' }}" />
                            </td>
                        @endforeach
                    </tr>
                @endfor
            ...
            @elseif (!empty($rows))
                ...
                @foreach ($rows as $row)
                    <tr class="{{ $rowClasses }} {{ $striped && $loop->even ? 'bg-[var(--zayne-color-base-200)]' : '' }}">
                        ...
                        @if ($selectable)
                            <td class="px-4 py-3">
                                <x-zayne.form.checkbox name="row_{{ $loop->index }}" size="sm" />
                            </td>
                        @endif
